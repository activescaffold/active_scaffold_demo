class Nature::ForestsController < ApplicationController

  active_scaffold :forest do |config|
    [:list, :create, :update, :search].each {|action| config.send(action).columns.exclude :lakes}
